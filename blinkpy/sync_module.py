...
class BlinkSyncModule:
    ...
    async def update_cameras(self, camera_type=BlinkCamera):
        ...
        try:
            ...
            for camera_config in self.camera_list:
                ...
                await self.cameras[name].update(camera_info, force_cache=True, force=True)
        except KeyError:
    ...
    async def get_events(self, **kwargs):
        ...
        response = await api.request_sync_events(self.blink, self.network_id, force=force)
        try:
    ...
    async def get_network_info(self):
        ...
        self.network_info = await api.request_network_update(self.blink, self.network_id)
        try:
    ...
    async def poll_local_storage_manifest(self, manifest_request_id=None, max_retries=4):
